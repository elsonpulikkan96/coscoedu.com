...
import { FiPhone, FiArrowUp, FiGift } from "react-icons/fi";
import { site, whatsappLink } from "../../config/site";
import { useLuckyDraw } from "../luckydraw/context";

export default function FloatingActions() {
  const [showTop, setShowTop] = useState(false);
  const { active: luckyDrawActive, openRegistration } = useLuckyDraw();
  useEffect(() => {
  ...
  return (
    <div className="fixed bottom-5 right-5 z-[60] flex flex-col items-end gap-3">
      {luckyDrawActive && (
        <button onClick={openRegistration} aria-label="Mega Lucky Draw — register & win"
          title="Mega Lucky Draw" className="group relative">
          <span className="absolute inset-0 animate-ping rounded-full bg-brand-500/60" aria-hidden />
          <span className="relative flex h-14 w-14 items-center justify-center rounded-full bg-brand-600 text-white shadow-lift transition group-hover:scale-105">
            <FiGift size={26} />
          </span>
        </button>
      )}
